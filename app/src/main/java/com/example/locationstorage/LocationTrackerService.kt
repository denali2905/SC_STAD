...
//            notificationManager.notify(2,notificationTwo.build())
    ...
    private fun sendAlert(isNotification:Boolean){
        ...
        locationManager.getGPSLocation() { lat, lon ->
            try {
                ...
                val smsManager: SmsManager = this.getSystemService(SmsManager::class.java)
                    ...
                    val message = if (isNotification) arrayListOf(
                        "Notification sent",
                        "I am here -> https://www.google.com/maps/search/?api=1&query=$lat,$lon"
                    )
                else arrayListOf(
                            "ATTENTION Denali, I may be in danger...\n Please reach out to me\n",
                            "I am here -> https://www.google.com/maps/search/?api=1&query=$lat,$lon")
            ...
            if ((c - it.c)<0.001 && (c - it.c)>-0.001)
            {
                if((point.latitude in it.lat1-0.0015..it.lat2+0.0015) || (point.latitude in it.lat2-0.0015..it.lat1+0.0015)) {
                    if ((point.longitude in it.lon1 - 0.0015..it.lon2 + 0.0015) || (point.longitude in it.lon2 - 0.0015..it.lon1 + 0.0015)) {
                        if (it.timeInMinutes in 15..1424) {
                            if ((point.timeInMinutes + routeError[routeIndex])%1439 in it.timeInMinutes - 15..it.timeInMinutes + 15) {
                                routeError[routeIndex] = it.timeInMinutes - point.timeInMinutes
                                return true
                            }
                        } else if (it.timeInMinutes < 15) {
                            if (((point.timeInMinutes + routeError[routeIndex])%1439 in (it.timeInMinutes - 15 + 1440)..1439)) {
                                routeError[routeIndex] =
                                    it.timeInMinutes - point.timeInMinutes + 1440
                                return true
                            } else if (((point.timeInMinutes + routeError[routeIndex])%1439 in 0..it.timeInMinutes + 15)) {
                                routeError[routeIndex] = it.timeInMinutes - point.timeInMinutes
                                return true
                            }
                        } else {
                        ...
                        if (sameRouteStartHour) {
                            if ((point.timeInUNIX - routeStartTime + routeError[routeIndex]*60000) in it.timeFromStart - 900000..it.timeFromStart + 900000) {
                                routeError[routeIndex] =
